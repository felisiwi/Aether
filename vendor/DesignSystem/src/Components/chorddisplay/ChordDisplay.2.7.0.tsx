 ...
 * v2.7.0: TopRow/BottomRow `flex: 0 0 auto` so rows hold space in column flex layout.
 */
export interface ChordDisplayProps {
  ...
  /** Highlight these pitch-class names on the keyboard while a hint is hovered. */
  onChordHintHover?: (notes: string[]) => void;
  onChordHintLeave?: () => void;
  className?: string;
...
export const ChordDisplay: React.FC<ChordDisplayProps> = ({
  ...
  const augmentHints = (hints: ChordCollectionHint[]) =>
    hints.map((hint) => ({
      ...hint,
      onHoverStart: onChordHintHover
        ? () => onChordHintHover(hint.missingNotes)
        : undefined,
      onHoverEnd: onChordHintLeave,
    }));
        ...
        <div style={innerStyle}>
          <div style={topRowStyle}>
            {shouldShowGeneralHints ? (
              <ChordCollection type="general" hints={augmentHints(chordHints)} />
            ) : null}
            {shouldShowProgressionHints && progressionHints ? (
              <ChordBar
                resolveHints={augmentHints(progressionHints.resolve)}
                tensionHints={augmentHints(progressionHints.tension)}
                moveHints={augmentHints(progressionHints.move)}
              />
            ) : null}
          </div>
