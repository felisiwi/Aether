...
/**
 * BasicButton v1.2.0 — Figma **Basic/Buttons** (`13669:55650`).
 ...
 *
 * v1.2.0: border is constant at strokeL (2 px) in ALL states. Pressed/focus heavier
 * stroke (strokeXl = 3 px) is rendered via box-shadow: inset so the button never
 * changes outer dimensions. Fixes layout shift when latching toggled pressed state.
 *
 * v1.1.0 archived at `Archive/BasicButton.1.1.0.tsx`.
  ...
  if (phase === 'disabled' || phase === 'disabledPressed') {
    ...
    if (variant === 'secondary') {
      ...
      if (phase === 'disabledPressed') {
        const bg = C.backdropOpacityAdaptiveOpacityDarkenedStrong;
        return { background: bg, borderColor: bg, borderWidth: bw };
      }
      const bg = C.backdropStatesDisabledSurface;
      return { background: bg, borderColor: bg, borderWidth: bw };
...
function labelColor(
  ...
  if (
    ...
    return semanticColors.buttonTextButtonText;
  }
...
export default function BasicButton({
  ...
  const constantBorder = layout.strokeL;
  const insetExtra = surf.borderWidth > constantBorder
    ? `inset 0 0 0 ${surf.borderWidth - constantBorder}px ${surf.borderColor}`
    : '';
  const focusGlow = showFocusChrome ? `0 0 0 3px ${focusRingLight}` : '';
  const combinedShadow = [insetExtra, focusGlow].filter(Boolean).join(', ') || 'none';
